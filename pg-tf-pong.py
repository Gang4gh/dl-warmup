...
tf.set_random_seed(np.random.randint(12345))
...
X = tf.placeholder(tf.float32, shape=[1, D])
W1 = tf.placeholder(tf.float32, shape=[D, H])
W2 = tf.placeholder(tf.float32, shape=[H])

# H1 = tf.nn.relu(tf.tensordot(X, W1, 1))
...
with tf.Session() as sess:
	while True:
		...
		tfh, tfy0 = sess.run([H1, Y0], feed_dict=values)
		if abs(aprob - tfy0) > 1e-4:
